...
class GenNoise(nn.Module):
    def __init__(self, channels=64, num_of_layers=1):
...
class Net(nn.Module):
    ...
    def forward(self,IR,VIS):
        ...
        for i in range(self.N):
            fea_LSTM,hidden = self.LSTM(fea_M+fea_1)
            ...
            if self.training:
                ...
                fea_clean_list.append(fea_1)
        # fea_LSTM, hidden = self.LSTM(fea_M + fea_1)
        Y_final = norm_1(self.decoder(fea_1))
...
def salency(x):
    return norm_1(((x).clamp(min=0.0001))-torch.mean((x).clamp(min=0.0001)))
def norm_1(x):
    max1 = torch.max(x)
    min1 = torch.min(x)
